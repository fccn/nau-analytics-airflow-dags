...
def dim_course_edition(cfg:dict) -> KubernetesPodOperator:
    ...
    arguments=[
        f"""
          ...
          local:///opt/spark/work-dir/src/gold/gold_dim_course_edition.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="dim_course_edition_gold",
    task_id="dim_course_edition_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def dim_organization(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_dim_organization.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="dim_organization_gold",
    task_id="dim_organization_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def dim_time(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_dim_time.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="dim_time_gold",
    task_id="dim_time_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def dim_user(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_dim_user.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="dim_user_gold",
    task_id="dim_user_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def fact_certificate_d(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_fact_certificate_d.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="fact_certificate_d_gold",
    task_id="fact_certificate_d_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def fact_course_edition_daily(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --master k8s://https://kubernetes.default.svc:443 \
          --deploy-mode cluster \
          --name fact_course_edition_daily_gold \
          --conf spark.kubernetes.container.image={cfg["docker_image"]} \
          --conf spark.kubernetes.namespace={cfg["namespace"]} \
          --conf spark.kubernetes.authenticate.driver.serviceAccountName=spark-role \
          --conf spark.kubernetes.submission.waitAppCompletion=true \
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_fact_course_edition_daily.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="fact_course_edition_daily_gold",
    task_id="fact_course_edition_daily_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def fact_course_enrollment_daily(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --master k8s://https://kubernetes.default.svc:443 \
          --deploy-mode cluster \
          --name fact_course_enrollment_daily_gold \
          --conf spark.kubernetes.container.image={cfg["docker_image"]} \
          --conf spark.kubernetes.namespace={cfg["namespace"]} \
          --conf spark.kubernetes.authenticate.driver.serviceAccountName=spark-role \
          --conf spark.kubernetes.submission.waitAppCompletion=true \
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_fact_course_enrollment_d.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="fact_course_enrollment_daily_gold",
    task_id="fact_course_enrollment_daily_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )

def fact_student_grades(cfg:dict) -> KubernetesPodOperator:
    return KubernetesPodOperator(
    namespace=cfg["namespace"],
    service_account_name='spark-role',
    # ✔ official spark image built for k8s
    ...
    arguments=[
        f"""
          ...
          --conf spark.kubernetes.executor.deleteOnTermination=true \
          --conf spark.kubernetes.container.image.pullPolicy=Always \
          local:///opt/spark/work-dir/src/gold/gold_fact_student_grades.py\
          2>&1 | tee log.txt; LAST_EXIT=$(grep -Ei "exit code" log.txt | tail -n1 | sed 's/.*: *//'); echo "Parsed Spark exit code: $LAST_EXIT"; exit "$LAST_EXIT"
        """
    ],
    name="fact_student_grades_gold",
    task_id="fact_student_grades_gold_1",
    get_logs=True,
    on_finish_action="delete_pod",
    dag = cfg["dag"]
    )
...
dim_course_edition_task = dim_course_edition(cfg=cfg)
dim_organization_task = dim_organization(cfg=cfg)
dim_time_task = dim_time(cfg=cfg)
dim_user_task = dim_user(cfg=cfg)
fact_certificate_d_task = fact_certificate_d(cfg=cfg)
fact_course_edition_daily_task = fact_course_edition_daily(cfg=cfg)
fact_course_enrollment_daily_task = fact_course_enrollment_daily(cfg=cfg)
fact_student_grades_task = fact_student_grades(cfg=cfg)
